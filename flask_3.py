...
lab_8 = Flask(__name__)

# Set a secret key to enable sessions
lab_8.secret_key = 'supersecretkey'
...
@lab_8.route('/register', methods=['GET', 'POST'])
def register():
...
@lab_8.route('/login', methods=['GET', 'POST'])
def login():
    if request.method == 'POST':
        ...
        if 'username' in session and 'password' in session and session['username'] == username and session['password'] == password:
            ...
            logging.info(f"Failed login attempt at {get_current_datetime()} from IP address {ip_address}. Username: {username}, Password: {password}")
